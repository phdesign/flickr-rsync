...
class CsvWalker(Walker):
    ...
    def walk(self):
        ...
        if self._config.root_files:
           source = source.start_with(*[(fileinfo, None) for fileinfo in self._storage.list_files(None)]) 
        if self._config.list_sort:
            source.to_sorted_list(key_selector=lambda (fileinfo, folder): (folder.name if folder else '', fileinfo.name)) \
